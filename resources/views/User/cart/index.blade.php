...
<x-app-layout>
    <div class="container mx-auto p-6">
        ...
        @if(Auth::user()->role === 'admin')
            ...
            <p class="text-gray-600">As an admin, you cannot access the cart feature.</p>
        @else
            ...
            @else
                            ...
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">
                                    <form action="{{ route('cart.remove', $cart->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-4 py-2 bg-red-500 text-white font-semibold rounded-md hover:bg-red-600 transition-colors">
                                            Remove
                                        </button>
                                    </form>
                                </td>
                ...
                <div class="mt-6 text-right">
                    <a href="{{ route('cart.checkout') }}" class="px-6 py-3 bg-green-500 text-white font-semibold rounded-lg shadow-md hover:bg-green-600 transition-colors">
                        Proceed to Checkout
                    </a>
                </div>
            @endif
